...
use plotters_dioxus::{DioxusCanvasDrawingArea, DioxusCanvasPlotter};
...
fn main() {
    // canvas only works on the web target because it uses web-sys
    LaunchBuilder::web().launch(App);
}

fn draw_scatter_plot(
    drawing_area: &mut DioxusCanvasDrawingArea,
    click_coord: ElementPoint,
    x_axis_scale: f64,
) -> () {
    let number_sample = 5000;
